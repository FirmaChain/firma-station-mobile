    ...
    const handleCW721ContractsInfo = useCallback(async () => {
        try {
            ...
            const currentNetworkContracts =
                cw721Contracts[address]?.filter((one) => one.network.toLowerCase() === network.toLowerCase()) || [];

            const list = await Promise.all(
                currentNetworkContracts.map(async (contract) => {
...
export const useCW20 = () => {
    ...
    const handleCW20ContractsInfo = useCallback(async () => {
        try {
            ...
            const currentNetworkContracts =
                cw20Contracts[address]?.filter((one) => one.network.toLowerCase() === network.toLowerCase()) || [];
